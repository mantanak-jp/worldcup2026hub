const dataFiles = {
  ...
  sourceRegistry: { path: "data/source_registry.json", optional: true, fallback: [] },
...
function listItems(items, emptyText = "まだありません") {
  ...
  return items.map((item) => `<li>${escapeHtml(labelFor(item))}</li>`).join("");
}
...
function labelFor(value) {
  const labels = {
    ...
    insufficient: "不足",
    insufficient_sources: "ソース不足",
    tactical_analysis: "戦術分析",
    longform_analysis: "ロングフォーム分析",
    statistics: "統計",
    official: "公式",
    match_report: "試合レポート",
    preview: "プレビュー",
    ...
    true: "あり",
    false: "なし",
    "approved source policy": "承認済みソースポリシー",
    "sufficient independent sources": "十分な独立ソース",
    "approved long-form source": "承認済みロングフォームソース",
    "approved statistics source": "承認済み統計ソース",
    "article-level event evidence": "記事単位のイベント根拠",
    "event data": "イベントデータ",
    "independent second source": "独立した第2ソース",
    needs_event_data: "イベントデータによる確認が必要",
    sample_disagreement_requires_review: "サンプルの相違点に人手確認が必要",
    single_source_unapproved_policy_with_opposing_sample: "未承認ポリシーかつ対立サンプルあり"
  };
  return labels[String(value)] || String(value ?? "");
}

function sourceList(sourceIds, sources, sourceRegistry = []) {
  const sourceMap = byId(sources);
  const registryMap = byId(sourceRegistry);
  const items = (sourceIds || []).map((id) => ({
    id,
    source: sourceMap.get(id),
    registry: registryMap.get(id)
  }));
  ...
  return items.map(({ id, source, registry }) => {
    const name = source?.name || registry?.name || id;
    const url = source?.url || registry?.base_url || "#";
    const type = source?.source_type || (registry?.source_category || []).map(labelFor).join(", ") || "未分類";
    const language = source?.language || (registry?.languages || []).join(", ") || "未定";
    const status = source?.checked_status || registry?.allowed_use || registry?.robots_policy_status || "未確認";
    const note = source?.japanese_note || registry?.notes || "メモはまだありません";

    return `
      <li>
        <a href="${escapeHtml(safeUrl(url))}" target="_blank" rel="noopener noreferrer">${escapeHtml(name)}</a>
        <p>${escapeHtml(type)} / ${escapeHtml(language)} / ${escapeHtml(labelFor(status))}</p>
        <p>${escapeHtml(note)}</p>
      </li>
    `;
  }).join("");
...
function joinLabels(items, emptyText = "未定") {
  return (items || []).map(labelFor).join(", ") || emptyText;
}
...
function formatSourceCoverage(coverage) {
  ...
  return [
    ["カバレッジ", labelFor(coverage.coverage_level || "未定")],
    ["ソース数", String(coverage.source_count ?? 0)],
    ["記事数", String(coverage.article_count ?? 0)],
    ["抽出数", String(coverage.extraction_count ?? 0)],
    ["言語", joinLabels(coverage.languages)],
    ["ソース種別", joinLabels(coverage.source_types)],
    ["承認済みソース", String(coverage.approved_source_policy_count ?? 0)],
    ["メタデータのみ", labelFor(String(Boolean(coverage.metadata_only_evidence)))]
  ];
}

function statusLabel(status) {
  return labelFor(status || "not_generated") || "Unknown";
}

function statusHelp(status) {
  if (status === "auto_published") {
    return "承認済みソースと十分な根拠がある場合だけ自動公開対象になります。";
  }
  if (status === "low_confidence") {
    return "表示はしますが、根拠量やclaim信頼度が低いため短いレビューとして扱います。";
  }
  if (status === "insufficient_sources") {
    return "独立ソースや承認済みポリシーが不足しているsample / early draftです。";
  }
  if (status === "blocked") {
    return "構造化claimや検証済み入力が不足しているため、本文生成を止めています。";
  }
  return "生成状態を明示し、利用者がレビュー品質を判断できるようにしています。";
}
...
function generatedReviewSections(review) {
  const sections = [
    ...
    ["キープレイヤー", "key_players", "key_players_ja"],
  ...
  return html || "<section><h3>情報不足</h3><p>この試合の本文生成に使える支持claimはまだありません。</p></section>";
}

function reviewStatusClass(status) {
  if (status === "auto_published") return "status-pill status-good";
...
function renderGeneratedReview(data, matchId) {
  ...
  if (!review) {
    panel.innerHTML = `
      ...
      <p>検証済みのローカル生成処理でこの試合のレビューdataが作成されると、ここに表示されます。</p>
    `;
    return;
  }
  ...
  panel.innerHTML = `
    ...
    <p><strong>sample / dry-run:</strong> このレビューはローカル検証用の自動生成案です。実クロールや外部記事本文の保存は行っていません。</p>
    ...
    <p class="review-status-note">信頼度は、参照できる構造化claim、抽出メモ、ソース数、相違点、不足入力から計算するローカル品質シグナルです。正確性の保証ではありません。</p>
    ${review.disagreement_summary_ja ? `<section class="missing-inputs"><h3>ソース間の相違点</h3><p>${escapeHtml(review.disagreement_summary_ja)}</p></section>` : ""}
    ...
    <section class="missing-inputs">
      <h3>不確実性</h3>
      <ul class="detail-list">${listItems(review.uncertainty, "不確実性の記録はありません")}</ul>
    </section>
...
function renderMatch(data, id) {
  ...
  const generatedReview = (data.generatedMatchReviews || []).find((item) => item.match_id === match.id);
  ...
  document.querySelector("[data-source-list]").innerHTML = sourceList([...sourceIds], data.sources, data.sourceRegistry);
...
function renderTeam(data, id) {
  ...
  document.querySelector("[data-team-matches]").innerHTML = relatedMatches.length
    ...
    : "<li>関連試合はまだありません</li>";
  document.querySelector("[data-source-list]").innerHTML = sourceList(team.source_ids, data.sources, data.sourceRegistry);
  document.querySelector("[data-update-history]").innerHTML = updateHistoryList(history);
}
